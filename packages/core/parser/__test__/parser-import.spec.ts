...
import { ParserState, parseSassImports } from '../parser-import'

describe('parse import', () => {
  test('parseSassImports: Initial -> At', () => {
    const { getCurState } = parseSassImports('@')
    expect(getCurState()).toBe(ParserState.At)
  })

  test('parseSassImports: At -> AtImport', () => {
    const { getCurState } = parseSassImports('@i')
    expect(getCurState()).toBe(ParserState.AtImport)
  })

  test('parseSassImports: At -> AtUse', () => {
    const { getCurState } = parseSassImports('@u')
    expect(getCurState()).toBe(ParserState.AtUse)
  })

  test('parseSassImports: At -> Initial', () => {
    const { getCurState } = parseSassImports('@a')
    expect(getCurState()).toBe(ParserState.Initial)
  })

  test('parseSassImports: AtUse -> Initial', () => {
    const { getCurState } = parseSassImports('@use;')
    expect(getCurState()).toBe(ParserState.Initial)
  })

  test('parseSassImports: AtImport -> Initial', () => {
    const { getCurState } = parseSassImports('@import;')
    expect(getCurState()).toBe(ParserState.Initial)
  })

  test('parseSassImports: AtUse -> StringLiteral', () => {
    const { getCurState, getCurImport } = parseSassImports('@use "')
    expect(getCurState()).toBe(ParserState.StringLiteral)
    expect(getCurImport()).toMatchObject({ type: 'use', path: '"', start: 5 })

    const { getCurState: getCurState1, getCurImport: getCurImport1 } = parseSassImports('@use \'')
    expect(getCurState1()).toBe(ParserState.StringLiteral)
    expect(getCurImport1()).toMatchObject({ type: 'use', path: '\'', start: 5 })
  })

  test('parseSassImports: StringLiteral -> concat string', () => {
    const { getCurState, getCurImport } = parseSassImports('@import "test')
    expect(getCurState()).toBe(ParserState.StringLiteral)
    expect(getCurImport()).toMatchObject({ type: 'import', path: '"test', start: 8 })

    const { getCurState: getCurState1, getCurImport: getCurImport1 } = parseSassImports('@use "test')
    expect(getCurState1()).toBe(ParserState.StringLiteral)
    expect(getCurImport1()).toMatchObject({ type: 'use', path: '"test', start: 5 })
  })

  test('parseSassImports: AtImport -> end', () => {
    const {
      imports: imports1,
      getCurState: getCurState1,
      getCurImport: getCurImport1,
    } = parseSassImports('@use "test";')
